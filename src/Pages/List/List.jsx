...
const List = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/hotel/allhotels?city=${destination}`)
        // console.log(res)
        setData(res.data)
  ...
  const handleClick = async () => {
    try {
      const minmax = await axios.get(`http://localhost:5000/hotel/allhotels?city=${destination}&min=${min || 0}&max=${max || 999}`)
      // console.log(minmax)
      setData(minmax.data)
